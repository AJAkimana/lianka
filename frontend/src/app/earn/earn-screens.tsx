...
import { referralAPI, earnAPI, notificationAPI as notifAPI, userAPI as uAPI } from '@/lib/api';
...
export function EarnPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([referralAPI.getDashboard(), earnAPI.getLoyalty()])
      .then(([r, l]) => { setRefData(r.data); setLoyalty(l.data); })
  ...
  const rankProgress = {
    'New Member': 0, Contributor: 20, Builder: 40,
    'Growth Partner': 60, 'Strategic Partner': 80, 'Elite Contributor': 100,
    ...
    <div className="screen">
      ...
      <div className="mx-4 mt-3 card">
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <img src="/images/rank-silver.png" alt="Current Rank"
          ...
          {rankLevel < 6 && (
            <div className="shrink-0 text-center">
              <img src="/images/rank-gold.png" alt="Next Rank"
        ...
        <div className="grid grid-cols-4 gap-2 mb-3">
          {[
            ...
            <div key={i} className="bg-[#0f0f0f] rounded-lg p-2 text-center">
              ...
              <p className="text-[#555] text-[9px] mt-0.5 leading-tight">{s.label}</p>
            </div>
          ))}
        </div>
      ...
      <div className="mx-4 mt-3 card">
        ...
        <div className="bg-[#0f0f0f] rounded-lg p-3 flex items-center gap-2 mb-3">
          <p className="text-[#00C853] text-xs font-mono flex-1 truncate">{refLink}</p>
          <button onClick={copyLink}><Copy size={14} className="text-[#555]" /></button>
        </div>
        <div className="flex gap-2 mb-3">
          <button onClick={copyLink} className="flex-1 btn-secondary py-2.5 text-xs">
            <Copy size={14} /> Copy Link
          </button>
          <button
            onClick={() => navigator.share?.({ url: refLink, title: 'Join Lianka' })}
            className="flex-1 btn-secondary py-2.5 text-xs">
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          {[
            { label: 'Total Referrals', value: refData?.total_referrals || 0, color: 'text-white' },
            { label: 'Active Referrals', value: refData?.active_referrals || 0, color: 'text-[#00C853]' },
            { label: 'Deposit Bonus', value: fmt.usd(refData?.deposit_bonus_total || 0), color: 'text-[#1565C0]' },
            { label: 'ROI Bonus', value: fmt.usd(refData?.roi_bonus_total || 0), color: 'text-[#7B1FA2]' },
          ].map((item, i) => (
        ...
        {rankLevel < 3 && (
          <div className="bg-[#F9A82510] border border-[#F9A82530] rounded-lg p-2.5 mb-3">
            <p className="text-[#F9A825] text-[11px]">
              📊 Referral limit: {refData?.total_referrals || 0}/5 used.
              Reach <strong>Builder</strong> rank for unlimited referrals.
            </p>
          </div>
        )}
        <div className="bg-[#00C85510] border border-[#00C85330] rounded-lg p-3 mb-3">
          <p className="text-[#00C853] text-[11px] font-semibold mb-1">Referral Earnings:</p>
          <p className="text-[#888] text-[11px]">• 2% bonus on every deposit by your referrals.</p>
          <p className="text-[#888] text-[11px]">• 0.1% daily from ROI earned by your referrals.</p>
        </div>
      ...
      <div className="mx-4 mt-3 card">
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-white text-sm font-semibold">Promotion Balance</p>
          ...
          <div className="text-right">
            ...
            <p className={`text-xs font-bold ${rankLevel >= 3 ? 'text-[#00C853]' : 'text-[#555]'}`}>
        ...
        {rankLevel >= 3 ? (
          <button
            onClick={() => toast.success('Promotion application submitted — admin will review.')}
          ...
          <div className="bg-[#55555510] border border-[#33333330] rounded-xl p-3">
            <p className="text-[#555] text-xs">
              Reach <strong className="text-white">Builder rank</strong> (5 cycles + 10 active referrals) to apply for promotion bonuses.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="mx-4 mt-3 mb-6 card">
        ...
        <div className="flex items-center gap-4 mb-4">
          <div className="relative w-20 h-20 shrink-0">
            <svg className="w-20 h-20 -rotate-90" viewBox="0 0 80 80">
              ...
              <circle cx="40" cy="40" r="34" fill="none" stroke="#00C853" strokeWidth="8"
                strokeDasharray={`${2 * Math.PI * 34 * (user?.loyalty_score || 0) / 100} ${2 * Math.PI * 34}`}
                strokeLinecap="round" />
            </svg>
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-white font-black text-sm">{user?.loyalty_score || 0}%</span>
            </div>
          </div>
          <div className="flex-1">
            {loyalty ? [
              { label: 'Completed Cycles (35%)', pct: loyalty.completed_cycles_score / 35 * 100 },
              { label: 'Redeposit Behavior (20%)', pct: loyalty.redeposit_score / 20 * 100 },
              { label: 'No Breach (20%)', pct: loyalty.no_breach_score / 20 * 100 },
              { label: 'Timeframe Discipline (10%)', pct: loyalty.timeframe_score / 10 * 100 },
            ].map((item, i) => (
              <div key={i} className="flex items-center gap-2 mb-1.5">
                <span className="text-[#555] text-[9px] w-24 shrink-0 leading-tight">{item.label}</span>
        ...
        <div className="bg-[#F9A82510] border border-[#F9A82530] rounded-lg p-2.5 mb-3">
          <p className="text-[#F9A825] text-[11px]">
            ⭐ 80% threshold: {Number(user?.loyalty_score || 0) >= 80
...
// ═══════════════════════════════════════════════════════════
import { notificationAPI as notifAPI } from '@/lib/api';
...
export function NotificationsPage() {
  ...
  const dotColors: Record<string, string> = {
    green: '#00C853', blue: '#1565C0', yellow: '#F9A825',
    red: '#C1121F', purple: '#7B1FA2', gray: '#555555',
  };
  ...
  return (
    <div className="screen">
      ...
      <div className="flex items-center justify-between px-4 py-3">
        ...
        {unread > 0 && (
          <button onClick={markAll} className="text-[#1565C0] text-xs font-semibold">
            Mark all as read
          </button>
        )}
      </div>
        ...
        <div className="pb-6">
          {notifications.map(n => (
            <button key={n.id} onClick={() => markOne(n.id)}
              ...
              <div className="flex-1">
                <p className={`text-sm leading-snug mb-0.5 ${!n.is_read ? 'font-semibold text-white' : 'text-[#aaa]'}`}>
                  {n.title}
                </p>
                <p className="text-[#666] text-xs leading-relaxed">{n.message}</p>
                <p className="text-[#444] text-[10px] mt-1">{fmt.ago(n.created_at)}</p>
              </div>
              {!n.is_read && <div className="w-2 h-2 rounded-full bg-[#7B1FA2] mt-2 shrink-0" />}
            </button>
          ))}
        </div>
      )}
...
// TRANSACTIONS SCREEN
// ═══════════════════════════════════════════════════════════
import { userAPI as uAPI } from '@/lib/api';
...
export function TransactionsPage() {
  ...
  const typeColor: Record<string, string> = {
    CREDIT: '#00C853', DEBIT: '#C1121F',
  };

  const typeIcon: Record<string, string> = {
    DEPOSIT: '⬇', WITHDRAWAL: '⬆', ROI: '⭐', REFERRAL_BONUS: '👥',
    PROMOTION_BONUS: '🎁', REINVESTMENT: '🔄',
  };
      ...
      {loading ? (
        ...
        <div className="pb-6">
          {items
            ...
            .map(item => (
              <div key={item.id} className="flex items-center gap-3 px-4 py-3.5 border-b border-[#111]">
                ...
                <div className="flex-1">
                  <p className="text-white text-sm font-semibold">{item.reference_type?.replace('_', ' ')}</p>
                  <p className="text-[#555] text-[11px]">{item.wallet_type} wallet</p>
                  <p className="text-[#444] text-[10px]">{fmt.ago(item.created_at)}</p>
                </div>
                <div className="text-right">
                  <p className={`font-bold text-sm`} style={{ color: typeColor[item.entry_type] }}>
                    {item.entry_type === 'CREDIT' ? '+' : '-'}{fmt.usd(item.amount)}
                  </p>
                  <p className="text-[#444] text-[10px]">Balance: {fmt.usd(item.balance_after)}</p>
                </div>
              </div>
            ))}
          <div className="flex items-center justify-center gap-4 py-4">
            <button disabled={page === 1} onClick={() => setPage(p => p - 1)}
              className="text-[#555] text-sm disabled:opacity-30">← Previous</button>
            <span className="text-[#555] text-xs">Page {page}</span>
            <button disabled={items.length < 50} onClick={() => setPage(p => p + 1)}
              className="text-[#555] text-sm disabled:opacity-30">Next →</button>
          </div>
        </div>
      )}
      <BottomNav />
