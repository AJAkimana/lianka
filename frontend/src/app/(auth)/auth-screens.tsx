  ...
  return (
    <div className="min-h-screen flex flex-col pb-8 relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <img src="/images/globe-bg.png" alt=""
      ...
      <div className="relative z-10 flex flex-col min-h-screen px-6">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center py-6">
          <div className="relative mb-6">
            ...
            <div className="relative w-24 h-24 rounded-2xl bg-black/60 border border-[#00C85360]
                            flex items-center justify-center backdrop-blur-sm">
              <span className="text-[#00C853] font-black text-5xl leading-none">L</span>
            </div>
          </div>
          <h1 className="text-5xl font-black text-white tracking-widest mb-3">LIANKA</h1>
          <p className="text-[#00C853] font-bold text-xl mb-0.5">Intelligent Capital.</p>
          <p className="text-[#00C853] font-bold text-xl mb-2">Sustainable Growth.</p>
          <p className="text-[#aaa] text-sm">Built for the long run.</p>

          {/* 4-feature icons — using real generated image */}
          <div className="mt-10 w-full">
            <img src="/images/feature-icons.png" alt="Features"
              className="w-full max-w-[320px] mx-auto" />
            <div className="grid grid-cols-4 gap-2 mt-2">
              {['Secure & Safe', 'Profitable', 'Sustainable', '24/7 Support'].map((label, i) => (
                <p key={i} className="text-white text-[10px] font-bold text-center">{label}</p>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3 bg-black/40 border border-[#ffffff15] rounded-xl p-3 mt-2 backdrop-blur-sm">
            ...
            <div>
              <p className="text-white text-xs font-semibold">Trusted by thousands of investors worldwide</p>
              <p className="text-[#aaa] text-[10px]">Transparent • Reliable • Built on Trust</p>
            </div>
          </div>

          <p className="text-[#888] text-[11px] text-center flex items-center justify-center gap-1.5">
            🔒 Lianka is <span className="text-[#00C853]">encrypted</span> and your data is protected.
          </p>
          <p className="text-[#555] text-[10px] text-center">© 2024 Lianka. All rights reserved.</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] px-6 pb-10">
      <div className="flex items-center justify-between pt-6 mb-6">
        ...
        <Link href="/login" className="text-[#00C853] text-sm font-semibold">Login</Link>
      </div>

      <div className="flex items-start justify-between mb-6">
        <div>
          <h2 className="text-2xl font-black text-white mb-1">Create Account</h2>
          <p className="text-[#666] text-sm">Join Lianka and start your investment journey.</p>
        </div>
        {/* Real wallet+shield graphic */}
        <img src="/images/register-graphic.png" alt="Secure Account"
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          ...
          <p className="text-[#555] text-[11px] mt-1 flex items-center gap-1">
            <span className="text-[#00C853]">✓</span> We will send a verification code to this email address.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="flex gap-3 mt-2 flex-wrap">
            {rules.map((r, i) => (
              <span key={i} className={`text-[11px] flex items-center gap-1 ${r.ok ? 'text-[#00C853]' : 'text-[#555]'}`}>
        ...
        <div>
          <label className="text-white text-sm font-semibold mb-1.5 block">
            Referral Code <span className="text-[#555] font-normal">(Optional)</span>
          </label>
          <div className="relative">
          ...
          <p className="text-[#555] text-[11px] mt-1">You will receive benefits when you use a referral code.</p>
          {/* Why referral code info box */}
          <div className="mt-2 bg-[#1565C010] border border-[#1565C030] rounded-xl p-3
                          flex items-start gap-2">
            <span className="text-[#1565C0] shrink-0">ℹ</span>
            <div>
              <p className="text-[#1565C0] text-xs font-semibold">Why referral code?</p>
              <p className="text-[#888] text-[11px]">You get special bonuses and rewards when you join using a referral code.</p>
            </div>
          </div>
        </div>
        ...
        <p className="text-[#333] text-[10px] text-center">© 2024 Lianka. All rights reserved.</p>
      </form>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-[#0a0a0a] px-6 pb-10">
      ...
      <div className="pt-6 mb-6">
        <div className="flex items-center justify-between">
          ...
          <span className="text-white font-bold text-sm">Email Verification</span>
        </div>
        {/* 4-step progress bar */}
        <div className="flex items-center gap-1 mt-4">
          {['Create Account', 'Verify Email', 'Set Security', 'Complete'].map((s, i) => (
            <div key={i} className="flex items-center flex-1">
              <div className={`flex items-center gap-1 ${i === 0 || i === 1 ? 'text-[#00C853]' : 'text-[#555]'}`}>
                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold border
                  ${i <= 1 ? 'bg-[#00C853] border-[#00C853] text-black' :
                    i === 2 ? 'border-white text-white' : 'border-[#333] text-[#555]'}`}>
                  {i <= 1 ? '✓' : i + 1}
                </div>
              ...
              {i < 3 && <div className={`flex-1 h-px mx-1 ${i < 1 ? 'bg-[#00C853]' : '#333'}`} />}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex justify-center mb-6">
        <div className="w-20 h-20 rounded-full bg-[#00C85515] border-2 border-[#00C85330]
                        flex items-center justify-center">
          <span className="text-4xl">✉️</span>
        </div>
      </div>

      <h2 className="text-2xl font-black text-white text-center mb-1">Verify Your Email</h2>
      <p className="text-[#888] text-sm text-center mb-1">We've sent a 6-digit verification code to</p>
      <p className="text-[#00C853] font-semibold text-sm text-center mb-2">{email}</p>
      ...
      <div className="bg-[#111] border border-[#1a1a1a] rounded-xl p-3 flex items-center justify-between mb-6">
        <div>
          <p className="text-white text-xs font-semibold">Didn't receive the code?</p>
          <p className="text-[#555] text-[11px]">Check your spam folder or resend the code.</p>
        </div>
      ...
      <button onClick={handleVerify} disabled={loading || token.join('').length < 6}
        className="w-full py-4 rounded-xl bg-[#00C853] text-black font-bold text-base
      ...
      <div className="bg-[#111] border border-[#1a1a1a] rounded-xl p-3 flex items-center gap-3">
        ...
        <div>
          <p className="text-white text-xs font-semibold">Your security is our priority</p>
          <p className="text-[#555] text-[11px]">All verification codes are encrypted and valid for a limited time only.</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#0a0a0a] px-6 pb-10">
        ...
        {['Create Account', 'Verify Email', 'Set Security', 'Complete'].map((s, i) => (
          <div key={i} className="flex items-center flex-1">
            <div className="flex items-center gap-1">
              <div className="w-6 h-6 rounded-full bg-[#00C853] border border-[#00C853]
      ...
      <div className="flex justify-center mb-8">
        <div className="relative">
          ...
          <div className="relative w-32 h-32 rounded-full bg-[#00C85520] border-2 border-[#00C853]
                          flex items-center justify-center">
      ...
      <h2 className="text-3xl font-black text-white text-center mb-1">Account Created</h2>
      <p className="text-[#00C853] font-bold text-2xl text-center mb-4">Successfully!</p>
      ...
      <div className="bg-[#111] border border-[#1a1a1a] rounded-xl p-4 grid grid-cols-3 gap-3 mb-6">
        {[
          { icon: '👤', label: 'Account Status', value: 'Active', color: 'text-[#00C853]' },
          { icon: '🛡', label: 'Security', value: 'Enabled', color: 'text-[#00C853]' },
          { icon: '📈', label: 'Your Journey', value: 'Just Started', color: 'text-[#00C853]' },
        ].map((item, i) => (
          <div key={i} className={`text-center ${i === 1 ? 'border-x border-[#222]' : ''}`}>
      ...
      <div className="bg-[#111] border border-[#1a1a1a] rounded-xl p-4 flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-[#666] text-xs">Explore our investment plans, make your first deposit, and start growing with Lianka.</p>
        </div>
      </div>
      ...
      <p className="text-[#444] text-[11px] text-center flex items-center justify-center gap-1.5 mb-4">
        🔒 Your data is <span className="text-[#00C853]">encrypted</span> and 100% secure.
      </p>

      {/* Earth footer strip matching Screen 04 design */}
      <div className="mx-0 -mx-6 overflow-hidden rounded-b-none">
        <img src="/images/earth-footer.png" alt=""
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      const payload = needs2FA
        ...
        : data;
      if (!needs2FA) { setSavedEmail(data.email); setSavedPassword(data.password); }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] px-6 pb-10">
      <div className="flex items-center justify-between pt-6 mb-8">
        ...
        <Link href="/register" className="text-[#00C853] text-sm font-semibold">Create Account</Link>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          ...
          <div className="relative">
            <input {...register('email', { required: true })} type="email"
              placeholder="Enter your email" className="input pl-10" />
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#555]">✉</span>
          </div>
        </div>
        <div>
        ...
        {needs2FA && (
          <div>
            <label className="text-white text-sm font-semibold mb-1.5 block">2FA Code</label>
        ...
        <div className="flex justify-end">
          <Link href="/forgot-password" className="text-[#555] text-xs hover:text-[#00C853]">
            Forgot password?
          </Link>
        </div>
