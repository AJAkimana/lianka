...
export default function AdminUsersPage() {
  ...
  const loadData = (s = search, state = stateFilter) => {
    setLoading(true);
    adminAPI.getUsers(1, 50, { search: s, state: state || undefined })
    ...
    <div className="min-h-screen bg-[#050505]">
      ...
      <div className="p-6">
        ...
        <div className="flex gap-3 mb-4">
          ...
          <select value={stateFilter} onChange={e => { setStateFilter(e.target.value); loadData(search, e.target.value); }}
            className="bg-[#111] border border-[#222] rounded-lg px-3 py-2.5 text-white text-sm
                       focus:outline-none focus:border-[#00C853]">
            <option value="">All States</option>
            {['ACTIVE','GRACE','INACTIVE','TERMINATED','FROZEN'].map(s => (
        ...
        {loading ? <div className="flex justify-center mt-20"><Spinner size={32} /></div> : (
          <div className="bg-[#111] rounded-xl border border-[#1a1a1a] overflow-hidden">
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#1a1a1a]">
                  {['User', 'State', 'Balance', 'Loyalty', 'Rank', 'Joined', ''].map(h => (
              ...
              <tbody>
                {users.map(u => (
                  <tr key={u.id} className="border-b border-[#111] hover:bg-[#161616] transition-colors">
                    <td className="px-4 py-3">
                      <p className="text-white text-sm font-medium">{u.full_name || '—'}</p>
                      <p className="text-[#555] text-xs">{u.email}</p>
                    </td>
                    <td className="px-4 py-3"><StatusBadge status={u.account_state} /></td>
                    <td className="px-4 py-3">
                      <p className="text-[#00C853] text-sm font-semibold">{fmt.usd(u.total_balance)}</p>
                      <p className="text-[#555] text-xs">AUM: {fmt.usd(u.active_deposit)}</p>
                    </td>
                    <td className="px-4 py-3">
                      <p className="text-white text-sm">{fmt.pct(u.loyalty_score)}</p>
                    </td>
                    <td className="px-4 py-3">
                      <p className="text-[#F9A825] text-xs font-semibold">{u.rank}</p>
                    </td>
                    <td className="px-4 py-3 text-[#555] text-xs">{fmt.date(u.created_at)}</td>
                    <td className="px-4 py-3">
            ...
            {users.length === 0 && (
              <div className="py-12 text-center text-[#555]">No users found</div>
            )}
          </div>
        )}
      </div>
